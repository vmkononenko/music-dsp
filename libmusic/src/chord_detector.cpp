...
vector<amplitude_t> SpecialConvolution(vector<amplitude_t> &convolvee, vector<amplitude_t> &kernel)
{
    float s;
    int m, n;
    ...
    vector<amplitude_t> Z(lenKernel, 0);

    if (lenKernel % 2 == 0) {
        throw runtime_error("SpecialConvolution(): lenKernel % 2 == 0");
    }

    for (n = lenKernel - 1; n < lenConvolvee; n++) {
        s = 0;
        for (m = 0; m < lenKernel; m++) {
            s += convolvee.at(n-m) * kernel.at(m);
        }
        Z.at(n - lenKernel / 2) = s;
    }

    // fill upper and lower pads
    for (n = 0; n < lenKernel / 2; n++) {
        Z.at(n) = Z.at(lenKernel / 2);
    }
    for (n = lenConvolvee; n < lenConvolvee + lenKernel / 2; n++) {
        Z.at(n - lenKernel / 2) = Z.at(lenConvolvee - lenKernel / 2 -  1);
    }
...
float ChordDetector::Tune_(tft_t *tft)
{
    ...
    vector<amplitude_t> mean_tunings(nBPS, 0), sinvalues, cosvalues;


    /* estimate mean tunings */
    for (auto & col : lsg) {
        for(int iTone = 0; iTone < round(col.size() * 0.62/nBPS)*nBPS+1; iTone = iTone + nBPS) {
            for(int iBPS = 0; iBPS < nBPS; ++iBPS) {
                mean_tunings[iBPS] += col[iTone + iBPS];
            }
        }
    }

    for (auto & val : mean_tunings) {
        val /= lsg[0].size();
    }
    ...
    for (int iBPS = 0; iBPS < nBPS; ++iBPS) {
        meanTuningReal += mean_tunings[iBPS] * cosvalues[iBPS];
        meanTuningImag += mean_tunings[iBPS] * sinvalues[iBPS];
    }
    ...
    float floatShift = normalisedtuning * 3 - intShift;

    for (auto & col : lsg) {
        typeof(col) currentTunedSpec;

        currentTunedSpec.push_back(0.0);
        currentTunedSpec.push_back(0.0); // set lower edge to zero

        for (uint32_t k = 2; k < col.size() - 3; ++k) { // interpolate all inner bins
            amplitude_t tempValue = col[k + intShift] * (1 - floatShift) + col[k + intShift + 1] * floatShift;
            currentTunedSpec.push_back(tempValue);
        }
        ...
        for (uint32_t i = 0; i < col_size; i++) { // first step: squared values into vector (variance)
            runningstd.push_back((currentTunedSpec[i] - runningmean[i]) * (currentTunedSpec[i] - runningmean[i]));
        }
        ...
        for (uint32_t i = 0; i < col_size; i++) {
            ...
            if (runningstd[i] > 0) {
                currentTunedSpec[i] = (currentTunedSpec[i] - runningmean[i]) > 0 ?
                    (currentTunedSpec[i] - runningmean[i]) / runningstd[i] : 0;
            }
            if (currentTunedSpec[i] < 0) {
